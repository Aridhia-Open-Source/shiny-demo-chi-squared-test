...
shinyServer(function(input, output) {
  ...
  test3 <- reactive({
    ...
    print(res$residuals/sqrt(outer(1-rowSums(x)/sum(x), 1-colSums(x)/sum(x)))) # ???????????????????????????
    
    cat("\n", "[p-values of adjusted standardized residuals (two-tailed)]", "\n")
    print(round(2*(1-pnorm(abs(res$residuals/sqrt(outer(1-rowSums(x)/sum(x), 1-colSums(x)/sum(x)))))),3)) # ??????????????????????????????(????????????)
